...
async function csrfAuth (ctx, next) {
  _csrfAuth(ctx)
}

async function refererAuth (ctx) {
  _refererAuth(ctx)
}
...
function _refererAuth (ctx) {
  if (process.env.NODE_ENV === 'development') {
    if (!/^https?:\/\/127\.0\.0\.1:3004/.test(ctx.headers['referer'])) {
      ctx.throw(403, 'FORBIDDEN: REFERER WRONG')
    }
  } else {
    if (!/^https?:\/\/127\.0\.0\.1:3002/.test(ctx.headers['referer'])) {
      ctx.throw(403, 'FORBIDDEN: REFERER WRONG')
    }
  }
}
